...
from utils import OpenAiBaseAgent, logger, getSessionID, setSessionIDHeader


logger = logger.setup_logger("software-developer-agent")


class SoftwareDeveloperAgent(OpenAiBaseAgent):
    def __init__(self, name, host, api_key, model):
        super().__init__(name, host, api_key, model)

    @observe()
    def execute(self, query: list[dict]) -> str:
        messages = (
            *query,
        )
        response = self.chat(
...
app = Flask(__name__)
agent = SoftwareDeveloperAgent('software-developer-agent', Env("OPEN_AI_URL").get(), "0p3n-w3bu!", Env("OPEN_AI_MODEL").get())
...
@observe(name="Software developer")
def get_response():
    data = request.json
